      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-start gap-4 flex-wrap">
            {fund.image && (
              <div className="w-20 h-20 rounded-xl overflow-hidden bg-white border border-slate-200 shadow-sm shrink-0">
                <img
                  src={fund.image}
                  alt={fund.name}
            ...
            <div>
              ...
              <p className="text-lg text-slate-600 mt-1">{fund.symbol}</p>
              <Badge variant="outline" className="mt-2">{fund.category}</Badge>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-6">
          <Card className="border-slate-200">
            ...
            <CardContent className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div>
                <p className="text-sm text-slate-600">NAV</p>
                <p className="text-lg font-semibold text-slate-900">₹{formatNumber(fund.nav, 2)}</p>
              </div>
              <div>
                <p className="text-sm text-slate-600">AUM</p>
                <p className="text-lg font-semibold text-slate-900">{formatAUM(fund.aum)}</p>
              </div>
              <div>
                <p className="text-sm text-slate-600">Expense ratio</p>
                <p className="text-lg font-semibold text-slate-900">{formatPercent(fund.expenseRatio)}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-slate-900">{formatPercent(fund.volatility)}</p>
              </div>
              <div>
                <p className="text-sm text-slate-600">Manager</p>
                <p className="text-sm font-medium text-slate-900">{fund.manager}</p>
              </div>
              <div>
                <p className="text-sm text-slate-600">Inception</p>
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className={`text-lg font-semibold ${getReturnColor(fund.returns.oneYear)}`}>
                    {fund.returns.oneYear >= 0 ? '+' : ''}{formatPercent(fund.returns.oneYear)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-slate-600">3 Year</p>
                  <p className={`text-lg font-semibold ${getReturnColor(fund.returns.threeYear)}`}>
                    {fund.returns.threeYear >= 0 ? '+' : ''}{formatPercent(fund.returns.threeYear)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-slate-600">5 Year</p>
                  <p className={`text-lg font-semibold ${getReturnColor(fund.returns.fiveYear)}`}>
                    {fund.returns.fiveYear >= 0 ? '+' : ''}{formatPercent(fund.returns.fiveYear)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-slate-600">10 Year</p>
                  <p className={`text-lg font-semibold ${getReturnColor(fund.returns.tenYear)}`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6">
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-semibold text-slate-900">Returns Estimator</h3>
                      <p className="text-sm text-slate-500">Estimation is based on the past performance</p>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="sip-amount" className="text-xs font-medium uppercase tracking-wider text-slate-500">
                      Enter Amount
                    </Label>
                    <div className="flex items-baseline rounded-lg border-2 border-sky-200 bg-white px-4 py-3 focus-within:border-sky-500 focus-within:ring-2 focus-within:ring-sky-500/20">
                      <span className="text-xl text-slate-400 align-bottom">₹ </span>
                      <Input
                        id="sip-amount"
                        type="number"
                        min={500}
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      <Label className="text-sm text-slate-700">Select Duration</Label>
                      <span className="text-sm font-medium text-slate-900">{sipYears} Yr{sipYears !== 1 ? 's' : ''}</span>
                    </div>
                    <Slider
                      value={[sipYears]}
                      ...
                      min={1}
                      max={30}
                      className="[&_[data-slot=slider-track]]:bg-slate-200 [&_[data-slot=slider-range]]:bg-sky-500"
                    />
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      <Label className="text-sm text-slate-700">Expected Rate of Return</Label>
                      <span className="text-sm font-medium text-slate-900">{sipReturn} %</span>
                    </div>
                    <Slider
                      value={[sipReturn]}
                ...
                {/* Right: Results */}
                {sipResult && (
                  <div className="flex flex-col justify-between rounded-lg bg-slate-50/80 border border-slate-200 p-6 lg:min-h-[320px]">
                    <div>
                      <p className="text-sm text-slate-600 mb-1">
                        The total value of your investment after <span className="font-semibold text-slate-900">{sipYears} Year{sipYears !== 1 ? 's' : ''}</span> will be
                      </p>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6 mb-6">
                      <div className="w-44 h-44 shrink-0">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={[
                                { name: 'Invested', value: sipResult.totalInvested, fill: '#f97316' },
                                { name: 'Returns', value: Math.max(0.01, sipResult.estimatedReturns), fill: '#7dd3fc' },
                              ]}
                              dataKey="value"
                              innerRadius={56}
                              outerRadius={72}
                      ...
                      <div className="space-y-3 flex-1 w-full sm:w-auto">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-xs font-medium text-slate-500">Invested Amount</p>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="h-4 w-1 rounded-full bg-sky-300 shrink-0" />
                          <div>
                            <p className="text-xs font-medium text-slate-500">Est. Returns</p>
                            <p className="text-sm font-semibold text-slate-900">
                    ...
                    <Button className="w-full bg-sky-600 hover:bg-sky-700 text-white font-medium py-6 rounded-lg">
                      INVEST NOW
                    </Button>
                  </div>
                )}
                ...
                {fund.holdings.slice(0, 10).map((holding, idx) => (
                  ...
                  >
                    <span className="font-medium text-slate-900">
                      {holding.name}
                      {holding.symbol && (
                        <span className="text-slate-500 ml-1">({holding.symbol})</span>
                      )}
                    </span>
                    <span className="text-slate-600">{formatPercent(holding.percentage)}</span>
                  </li>
                ))}
              </ul>
            </CardContent>
